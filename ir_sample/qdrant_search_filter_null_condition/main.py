...
def main():
    ...
    condition = SearchCondition(color="blue", is_null_city=True)
    print(condition)
    query = QueryBuilder.build(condition=condition, vector=[0.9, 0.1, 0.1])
    response = client.search(collection_name, query)
    show(response)

    condition = SearchCondition(is_null_city=True)
    print(condition)
    query = QueryBuilder.build(condition=condition, vector=[0.9, 0.1, 0.1])
    response = client.search(collection_name, query)
    show(response)

    condition = SearchCondition(is_null_color=True)
    print(condition)
    query = QueryBuilder.build(condition=condition, vector=[0.9, 0.1, 0.1])
    response = client.search(collection_name, query)
    show(response)

    condition = SearchCondition(city="Tokyo", is_null_city=True)
    print(condition)
    query = QueryBuilder.build(condition=condition, vector=[0.9, 0.1, 0.1])
    response = client.search(collection_name, query)
    show(response)

    condition = SearchCondition(is_null_color=True, is_null_city=True)
    print(condition)
    query = QueryBuilder.build(condition=condition, vector=[0.9, 0.1, 0.1])
    response = client.search(collection_name, query)
    show(response)
