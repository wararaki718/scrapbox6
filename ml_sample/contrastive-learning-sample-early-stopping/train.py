...
class Trainer:
    def __init__(self, n_epochs: int=10, early_stopping: bool=True, stop_counts: int=10) -> None:
        self._n_epochs = n_epochs
        self._criterion = TripletContrastiveLoss()
        self._early_stopping = early_stopping
        self._stop_counts = stop_counts
    ...
    def train(
        ...
        for epoch in range(1, self._n_epochs + 1):
            ...
            counter += 1
            if self._stop_counts <= counter:
                print(f"early stopping: {best_valid_loss}")
                break
