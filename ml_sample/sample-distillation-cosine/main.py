...
from model import ModifiedDeepNNCosine, ModifiedLightNNCosine, NNModel
...
def main() -> None:
    ...
    modified_nn_model = try_gpu(ModifiedDeepNNCosine(n_classes=10))
    modified_nn_model.load_state_dict(nn_model.state_dict())

    torch.manual_seed(42)
    distil_model = try_gpu(ModifiedLightNNCosine(n_classes=10))
